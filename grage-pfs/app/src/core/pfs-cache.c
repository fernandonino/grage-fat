...
#include "pfs-state.h"
	...
	void pfs_cache_put_sectors(DiskSector * sectorNuevo, List listaCacheSectors,
		...
		if (commons_list_getSize(listaCacheSectors) >= sectorsMaxCount) {
			...
			while (commons_iterator_hasMoreElements(fuckingIterator)) {
				if (nodo->estado
						<= ((CacheSectorRecord *) commons_iterator_next(
								fuckingIterator))->estado) {
					nodo = (CacheSectorRecord *) commons_iterator_next(
							fuckingIterator);
				}
			}
	...
	void pfs_cache_sectores_dumpBIS(List listaCacheSectors, uint32 sectorsMaxCount) {
		...
		while (commons_iterator_hasMoreElements(fuckingIterator)) {
			...
			fwrite(nodo->sector.sectorContent, sizeof(char),
					strlen(nodo->sector.sectorContent), cache_dump);
			i = strlen(linea);
			while (i < sizeof(linea)) {
				if (linea[i] == '\0') {
					fputc(' ', cache_dump);
